...
const script = "process.stdin.setEncoding('utf8'); let b=''; process.stdin.on('data', c => { b+=c; for (;;) { const i=b.indexOf('\\n'); if(i<0)break; const m=JSON.parse(b.slice(0,i)); b=b.slice(i+1); if(m.method==='ask') process.stdout.write(JSON.stringify({jsonrpc:'2.0',id:99,method:'approval',params:{}})+'\\n'); else if(m.id===99) process.stdout.write(JSON.stringify({jsonrpc:'2.0',id:1,result:{accepted:true}})+'\\n'); } });"
