...
- `deepseek-ai/deepseek-harness` 仅作为外部依赖和接口参考，禁止修改其源码、提交其仓库或在其目录内开发。
- 所有新增代码、测试、构建脚本和文档只能提交到 `dsh-agent-plugin`。
- 最终部署目录固定为 `dsh-codex`；运行时由该目录加载插件并连接已安装的 DSH/Codex/Mixin，不覆盖 DSH 原文件。
- 插件必须通过 DSH 已公开的插件、profile、API 或 RPC 扩展点接入；若某项能力没有公开扩展点，应先增加适配层，不得通过修改 DSH 源码实现。
- 发布包应包含插件产物、配置模板、迁移/检查脚本、启动脚本和版本兼容矩阵，不包含 `deepseek-harness` 源码、`codex-mixin` 源码或用户密钥。
...
dsh-codex
  ├─ DSH（用户预先安装）
  ├─ Codex CLI/app-server（用户预先安装）
  └─ Codex Mixin gateway（用户预先安装，可选）
```
