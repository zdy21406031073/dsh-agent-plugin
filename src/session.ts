...
export class CodexSession {
  ...
  async start(prompt: string): Promise<void> { const result = await this.client.request('thread/start', { cwd: this.options.cwd, model: this.options.model ?? null, approvalPolicy: this.options.approvalPolicy ?? 'on-request', sandbox: this.options.sandbox ?? 'workspace-write' } as Json); this.threadId = readString(result, 'threadId'); await this.turn(prompt) }
  async resume(threadId: string, prompt: string): Promise<void> { this.threadId = threadId; await this.turn(prompt) }
  ...
  async stop(): Promise<void> { if (this.threadId) await this.client.request('turn/interrupt', { threadId: this.threadId }); this.status = 'idle' }
  async close(): Promise<void> { await this.client.close() }

  private async turn(prompt: string): Promise<void> { if (!this.threadId) throw new Error('Session has not started'); this.status = 'running'; try { await this.client.request('turn/start', { threadId: this.threadId, input: [{ type: 'text', text: prompt }] }); this.status = 'completed' } catch (error) { this.status = 'failed'; throw error } }
}
